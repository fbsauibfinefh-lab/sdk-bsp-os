...
def parser() -> argparse.ArgumentParser:
    ...
    resolve.add_argument(
        "--method",
        choices=[
            "weighted", "learned", "hybrid", "operation-weighted",
            "operation-semantic", "operation-structured", "operation-lambdamart",
        ],
        default="weighted",
    )
    resolve.add_argument("--model", type=Path)
    resolve.add_argument("--hybrid-weight", type=float)
    resolve.add_argument("--operation-min-margin", type=float, default=0.0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "resolve":
        value = SemanticResolver().resolve(
            ...
            operation_min_margin=args.operation_min_margin,
        )
