...
本文档记录 2026-08-23 至 2026-08-24 在 WSL Ubuntu 22.04、`AIoT-v1.0` 环境完成的构建实验。下表已更新为 v0.5 混合前端、目标感知闭包和统一自测协议接入后的结果。六个组合均从 SDK 摄取开始运行完整流水线并通过固件静态验证；结果不包含开发板运行验证。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| K210 | 201 | 646 | 2,076 | 23 | 59 | 9 | 1 / 1 |
| STM32CubeF1 | 3,921 | 12,292 | 42,565 | 12 | 377 | 0 | 0 / 0（后端提供） |
| PSoC E84 | 3,254 | 14,434 | 57,080 | 16 | 694 | 23 | 0 / 0（后端提供） |

## 构建结果

| 芯片/开发板 | RTOS | 架构 | 尝试 | ELF | 可刷写镜像 | text/data/bss（B） | 结果 |
| --- | --- | --- | ---: | ---: | ---: | --- | --- |
| K210 | RT-Thread | ELF64 RISC-V | 2 | 667,928 B | BIN 471,912 B | 467971 / 3864 / 45923 | 通过 |
...
| STM32F103 | Zephyr 4.4 | ELF32 ARM | 1 | 546,008 B | BIN 25,400 B | 25296 / 104 / 4046 | 通过 |
| PSoC E84 Edgi-Talk | RT-Thread | ELF32 ARM | 1 | 1,455,568 B | HEX 180,167 B | 62664 / 1368 / 258285 | 通过 |
| PSoC E84 兼容基线 | Zephyr 4.4 | ELF32 ARM | 1 | 923,580 B | BIN 44,624 B | 42500 / 2096 / 4581 | 通过 |
...
规范化绑定计划包含 19 个常见操作。K210 自动推断 19/19，STM32CubeF1 为 11/19，PSoC E84 为 14/19。后两项仍能编译是因为成熟 RTOS BSP 提供了已审计驱动，这不能反推自动绑定已经完整；正式论文应把自动计划覆盖与人工/成熟后端上限分开报告。
