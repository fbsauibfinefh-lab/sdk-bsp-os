...
本文档保留 v0.6 的能力级实验和历史结果。v0.7 已扩展为 20 个独立开发来源、19 个操作级查询和三套板卡外部测试；v0.8 进一步实现冻结 MiniLM 的低秩查询适配与运行时三路融合。最新协议与结果见 [IR 感知轻量语义排序方案 v0.8](ir-semantic-reranking-v0.8.md)。不同版本的查询单位和真值分母不能直接合并为同一张主表。
