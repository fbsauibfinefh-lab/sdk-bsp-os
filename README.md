...
当前论文部署方法为结构先验与目标可行性保护增强的 LambdaMART。K210 部署候选由 IR 独立生成，不读取目标板真值；系统对 14,814 个候选行计算字段语义、代码双视图、寄存器效果图和结构特征，并保留每项操作的完整候选排名。事后独立真值评测覆盖 48/48 个真值符号，原始模型 P@1 为 0.895、nDCG@10 为 0.816。部署解码器再用参数签名可封装性和同能力 API 家族完整性构造 19 项规范计划，避免把高分但签名不满足操作契约的函数直接写入后端。RT-Thread 的 19 项操作均直接调用计划所选 SDK 实体；Zephyr 有 15 项直接调用，4 项 PLIC 操作由 Zephyr 二级 IRQ 框架等价实现并在清单中明确标记。两套计划驱动固件均已通过目标编译和 ELF/BIN 验证；实板历史基线为两套各 10/10 次启动、90/90 条命令通过，计划驱动固件正在按同一协议重新复测。Zephyr 路径把恢复所得 SDK 绑定生成为 `clock_control`、UART、GPIO 和 Counter 四类原生 `struct device` 驱动，PLIC 则按 Zephyr 中断控制器子系统接入；验证程序只通过 Zephyr 标准 API 访问这些实现。
